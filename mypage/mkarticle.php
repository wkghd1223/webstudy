...
<body>
    ...
    <article>
        <form enctype='multipart/form-data' action='mkarprocess.php' method='post'>
        <?php
            $menu = mysqli_query($conn,'select * from menu');
            echo '<select class="custom-select" id="selectCG" name= "category">';
            $num = 1;
            while($row = mysqli_fetch_assoc($menu)){    
        ...
        ?>
        <br>
        이미지 :<input type='file' name='myfile' class="btn btn-light" ><br>
